...
def main(args):
    rec_file_path = os.path.join(args.image_dir, 'rec.txt')
    IN_VALIDATION_MODE = os.path.isfile(rec_file_path)
    if IN_VALIDATION_MODE is True:
        image_file_list = []
        gt_list = []
        ...
        for line in content:
            ...
            image_file_list.append(
                os.path.join(args.image_dir, line_info[0])
            )
    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        if IN_VALIDATION_MODE:
            valid_gt_list.append(gt_list[idx])
        img_list.append(img)
    try:
        rec_res, inference_time = text_recognizer(img_list)
    ...
    if IN_VALIDATION_MODE:
        ...
        for raw_res, conf_score in rec_res:
            pred = ''
            upper_res = raw_res.upper()
            for char_ in upper_res:
                if char_ in CHARS:
                    pred += char_
            dt_list.append(pred)
            text_to_file = '{}\t{}\n'.format(
                valid_image_file_list[idx],
                pred
            )
        ...
        for gt, dt in zip(valid_gt_list, dt_list):
            num_true_pred += gt == dt
        
        logger.info("Acc: {:.2f}".format(num_true_pred / len(dt_list) * 100))
        logger.info("FPS: {:.2f}".format(len(img_list) / inference_time))
    else:
